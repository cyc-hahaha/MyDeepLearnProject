...
# 初始化模型并移动到GPU
model = BiGRUModel(vocab_size, embedding_dim, hidden_dim, output_dim, dropout_rate).to(device)
...
for epoch in range(num_epochs):
    ...
    l_epo.append(epoch)
# 加载并测试最佳模型
# best_model = BiGRUModel(vocab_size, embedding_dim, hidden_dim, output_dim).to(device)
# best_model.load_state_dict(torch.load('best_model.pt'))
# best_model.eval()
# test_predictions = predict(best_model, X_test)
# test_accuracy, test_f1 = calculate_metrics(test_predictions, y_test)
# print(f"Best Model Test Accuracy: {test_accuracy:.4f} | Best Model Test F1: {test_f1:.4f}")
